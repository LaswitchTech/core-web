...
final class TwilioPlugin
{
    /**
     * Register the Twilio SMS provider with the registry.
     *
    ...
    public static function registerProvider(array $context): void
    {
        ...
        try {
            ...
            $twilioConfig = Config::get('sms.providers.twilio', []);
            
            // Ensure config is an array
            if (!\is_array($twilioConfig)) {
                $twilioConfig = [];
            }
            
            // Register the provider with priority 0 (default)
            $provider = new TwilioProvider($twilioConfig);
