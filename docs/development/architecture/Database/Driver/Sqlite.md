...
4. **Path resolution**:
   - Absolute paths (via `isAbsPath()` helper) are used directly.
   - Relative paths are joined with `$basePath` (after `\rtrim($basePath, '/')` and `\ltrim($path, '/')`).
5. **Directory creation** — Calls `\dirname($resolvedPath)` and checks if the parent directory exists via `\is_dir()`. If not, attempts `\mkdir($parentDir, 0755, true)`. The `@` suppressor prevents warning noise; failure produces a `DatabaseException`.
