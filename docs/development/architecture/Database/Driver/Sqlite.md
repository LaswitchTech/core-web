...
No constructor — the `connect()` method acts as a factory that takes `$config` and returns a fully initialized `Connection`. This keeps the driver lightweight (zero-instance overhead if never resolved) and avoids state leakage between calls.
...
4. **Path resolution**:
    - Absolute paths (via the class's private static `isAbsPath()` method) are used directly.
    - Relative paths are joined with `$basePath` (after `\rtrim($basePath, '/')` and `\ltrim($path, '/')`): `"{$basePath}/{$path}"`.
...
### `isAbsPath(string $path): bool` (private static)
...
**Regex breakdown (Windows detection):**
```
#[A-Za-z]:[/\\\\]|^\\\\[|\\\\\\\\[?\?\\\\]#
```
...
|-------------------------|----------------------------------|-----------------------|
| `[A-Za-z]:[/\\]`       | Windows drive letter + slash     | `C:\`, `D:/`          |
| `^\\\\[`                | UNC-style prefix (`\\server…`)   | `\\server\share`      |
| `\\\\[?\?\\\\`          | Win32 extended-length prefix     | `\??\C:\Windows\…`    |

For relative paths, resolution is simple string concatenation — no symlink following or canonicalization.
